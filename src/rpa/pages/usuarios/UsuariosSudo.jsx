...
export const UsuariosSudo = () => {
  const dispatch = useDispatch();
  
  const MySwal = withReactContent(Swal);
  const { usuarios } = useSelector((state) => state.rpa);
  ...
  const { tribunales } = useSelector((state) => state.rpa);
  ...
  const onChangeTribunal = (value) => {
    setTribunal(value);
    setUsr(usuarios.filter((usuario) => usuario.id_tribunal == value));
  };
  ...
  const onDeleteUsuario = ({ id_usuario, nombre, apellido }) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setUsr(usr.filter((usuario) => usuario.id_usuario !== id_usuario));
      }
    });
  };

  useEffect(() => {
    dispatch(setNavbarText('Administración de usuarios global'))
  }, []);
                  ...
                  <TableHead>
                    <StyledTableRow>
                      <StyledTableCell align="left" colSpan={3}>
                        {`TOTAL DE USUARIOS: ${usr.length}`}
                      </StyledTableCell>
                      <StyledTableCell align="right" colSpan={2}>
                        {trib !== '' && (
                          <Link
                            to={`/usuarios/addusuarioSudo/${trib}`}
