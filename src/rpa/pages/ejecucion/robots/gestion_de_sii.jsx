...
import { setNavbarText, startExeGestionDeSii } from "../../../../store/app";
import { useEffect, useState } from "react";
import { useForm } from "../../../../hooks/useForm2";
import { useParams } from "react-router-dom";
import HelpRoundedIcon from "@mui/icons-material/HelpRounded";
import "../../../../styles/robotHelp.css";
import { InsGestSii } from "../../../components/instructivos";
import Swal from "sweetalert2";
import withReactContent from "sweetalert2-react-content";

const gestion_de_sii = () => {
  ...
  const { instructivoGestSii } = InsGestSii(state, toggleDrawer);
  ...
  const formValidations = {
    ...
    user_sii: [
      [
        ...
        "Usuario Clave única debe tener más de 3 letras.",
      ],
    ],
    pass_sii: [
      [
        (value) => value.length >= 3,
        "Contraseña Clave única debe tener más de 3 letras.",
      ],
    ],
  };
  ...
  const onSubmit = (event) => {
    ...
    dispatch(startExeGestionDeSii({ ...formState }));
  };

  useEffect(() => {
    dispatch(setNavbarText("Gestión de SII"));
  }, []);
            ...
            >
              <form onSubmit={onSubmit} encType="multipart/form-data">
                <Grid container spacing={0}>
                  ...
                  <Grid item xs={12} sx={{ marginTop: 2 }}>
                    <TextField
                      label="Usuario Clave única"
                      type="text"
                      placeholder="Usuario Clave única"
                      fullWidth
                      name="user_sii"
                      variant="outlined"
                      value={user_sii}
                      onChange={onInputChange}
                      error={!!user_siiValid && formSubmitted}
                      helperText={user_siiValid}
                      disabled={disableForm}
                    />
                  </Grid>
                  <Grid item xs={12} sx={{ marginTop: 2 }}>
                    <TextField
                      label="Contraseña Clave única"
                      type="password"
                      placeholder="Contraseña Clave única"
                      fullWidth
                      name="pass_sii"
                      variant="outlined"
                      value={pass_sii}
                      onChange={onInputChange}
                      error={!!pass_siiValid && formSubmitted}
                      helperText={pass_siiValid}
                      disabled={disableForm}
                    />
                  </Grid>
                  <Grid item xs={12} sx={{ marginTop: 2 }}>
